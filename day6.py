...
# Part one answer
print(sum(map(len,[set(ans.replace('\n','')) for ans in  input.strip().split('\n\n')])))
# 6310

# part 2
'''
-- count how many times each letter occurs
-- if the count is same as number of people, count the letter
'''
...
for ans in input_list:
    total = total + len( [x for  x, count in Counter(''.join(ans)).most_common() if count == len(ans)])
