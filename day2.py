...
def process(input):
    return [(re.findall(numbers, line)[0]
        , re.findall(letter, line)[0][0]
        , line.split(':')[1].strip()
    )
...
def validate(rule, letter, password):
    min, max = rule.split('-')
    return int(min) <= password.count(letter) <= int(max)

x = process(input)
answer = sum([
    validate(*(rule, letter, password))
    for rule, letter, password in x
])
...
def validate_new(rule, letter, password):
    first, second = rule.split('-')
    return (password[int(first)-1] == letter) ^  (password[int(second)-1] == letter)

# print(validate_new('1-3', 'a', 'abcde'))

part_two_answer = sum([
    validate_new(*(rule, letter, password))
    for rule, letter, password in x
])
