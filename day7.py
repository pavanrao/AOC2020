...
"""
Solution approach: Convert input text to a nested dictionary and then search
Sample data would be translated to a dictionary like below:
...
# text = sample.strip().replace("bags",'').replace("bag","").replace(".","")

text = input.strip().replace("bags",'').replace("bag","").replace(".","")

bags = dict()


def split_inner_bags(inner):
    if "no other" in inner:
        return {None:0}
    else:
        result = dict()
        for inner_bags in inner.split(','):
            x = inner_bags.strip().split(' ', maxsplit=1)
            result[x[1]] = int(x[0])
        return result
...
for line in text.splitlines():
    detail = line.split("contain")
    bags.update( {detail[0].strip() : split_inner_bags(detail[1])})


def find_gold(d:dict, collected_gold:list = None):
    if collected_gold is None:
        all_gold = []
    else:
        all_gold = collected_gold

    for k, v in d.items():
        if k == "shiny gold":
            all_gold.append(d)
        elif k is not None:
            find_gold(bags[k],all_gold)

    return all_gold

gold = [1 for k,v in bags.items() if find_gold(v)]


print(f'bag colors can eventually contain at least one shiny gold bag = {sum(gold)}')
...
# pass the bag count as a multiplier every time the function is called

def find_bags_in_me(me:dict, collected_bags:list = None, multiplier=1):
    if collected_bags is None:
        all_bags = []
    else:
        all_bags = collected_bags

    for k, v in me.items():
        all_bags.append(v*multiplier)
        if k is not None:
            find_bags_in_me(bags[k],all_bags, v*multiplier)

    return all_bags



bags_in_gold_bag = find_bags_in_me(bags['shiny gold'])
# print(bags_in_gold_bag)
print(f'Part 2 - bags in the gold bag = {sum(bags_in_gold_bag)}')
